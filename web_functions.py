...
def load_data():
    ...
    sheet_id = "1fElncOrx03marELtaA76MdFjJCz_y3WC1waz7Ft3tRc"  # Extract sheet ID
    sheet_name = "cardiac"  # Replace with your sheet name, or remove if it's the first sheet

    url = f"https://docs.google.com/spreadsheets/d/{sheet_id}/gviz/tq?tqx=out:csv&sheet={sheet_name}"

    try:
        df = pd.read_csv(url)
        X = df[["Gender", "Chain_smoker", "Consumes_other_tobacco_products", "HighBP", "Obese", "Diabetes",
              "Metabolic_syndrome", "Use_of_stimulant_drugs", "Family_history", "History_of_preeclampsia",
              "CABG_history", "Respiratory_illness"]]
        y = df['UnderRisk']

        return df, X, y
    except Exception as e:
        st.error(f"Error loading data: {e}")
        return None, None, None  # Return None values to prevent further errors
...
def train_model(X, y):
    model = DecisionTreeClassifier(
        ...
        # If the impurity decrease from a split is less than this value, the split will not be performed.
